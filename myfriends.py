...
# ------------ BEGIN YOUR CODE ------------
        ...
        for line in infile:
            # split() with no arguments splits on runs of any whitespace
            # and discards leading/trailing whitespace, so empty lines
            # produce an empty list and are skipped.
            names = line.split()
            if len(names) == 2:
...
def find_all_number_of_friends(my_dir):
    ...
    # decreasing by number of friends; break ties in ascending ASCII
    # order of the person's name (so e.g. Han before Leia when tied).
    friends_list.sort(key=lambda item: (-item[1], item[0]))
...
def make_team_roster(person, my_dir):
    ...
    team = set()
    # first circle: immediate friends of the leader
    for friend in my_dir[person]:
        team.add(friend)
        # second circle: immediate friends of each friend
        for fof in my_dir[friend]:
            team.add(fof)
    # the leader appears only as the first substring
    team.discard(person)
    # subsequent substrings are the unique members in ASCII order
    for member in sorted(team):
        label = label + '_' + member
...
def find_smallest_team(my_dir):
    ...
    for person in my_dir:
        ...
        # size = number of people encoded in the roster label
        size = len(label.split('_'))
        rosters.append((size, label))
    # smallest size first; break ties by ASCII order of the roster label
    rosters.sort(key=lambda item: (item[0], item[1]))
    smallest_teams = [label for size, label in rosters]
